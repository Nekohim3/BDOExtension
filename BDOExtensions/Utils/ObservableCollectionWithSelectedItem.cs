...
namespace BDOExtensions.Utils
{
    public class ObservableCollectionWithSelectedItem<T> : ObservableCollection<T> where T : class
    {
        ...
        public bool SetSelectedToId(int id)
        {
            ...
            {
                if (!int.TryParse(prop.GetValue(x)?.ToString(), out var res) || res != id)
                    continue;
    ...
    public class ObservableCollectionWithSelectedItemNumber<T> : ObservableCollection<T> where T : INumber<T>
    {
        #region EventDelegate

        public delegate void                 SelectedChangedHandler(ObservableCollectionWithSelectedItemNumber<T> sender, T newSelection, T oldSelection);
        public event SelectedChangedHandler? SelectionChanged;
        ...
        public ObservableCollectionWithSelectedItemNumber() : base()
        {
            _filterTimer = new Timer(TimerCallback, null, Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
            CollectionChanged += OnCollectionChanged;
            SetSelectedToFirst();
        ...
        public ObservableCollectionWithSelectedItemNumber(IEnumerable<T> list) : base(list)
        {
            _filterTimer = new Timer(TimerCallback, null, Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
            _sourceCollection = Items.ToList();
            CollectionChanged += OnCollectionChanged;
        ...
        private void OnCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                case NotifyCollectionChangedAction.Add:
                    _sourceCollection.Insert(e.NewStartingIndex, (T)e.NewItems[0]);
                    break;
                case NotifyCollectionChangedAction.Move:
                    _sourceCollection.RemoveAt(e.OldStartingIndex);
                    _sourceCollection.Insert(e.NewStartingIndex, (T)e.NewItems[0]);
                    break;
                case NotifyCollectionChangedAction.Remove:
                    _sourceCollection.Remove((T)e.OldItems[0]);
                    break;
                case NotifyCollectionChangedAction.Replace:
                    _sourceCollection.RemoveAt(e.OldStartingIndex);
                    _sourceCollection.Insert(e.NewStartingIndex, (T)e.NewItems[0]);
